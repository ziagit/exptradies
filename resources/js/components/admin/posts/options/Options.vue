<template>
  <div class="">
    <b-modal id="modal-update" title="Update option" :hide-footer="true" hide-header-close>
      <Edit v-on:close-dialog="refresh" :option="option" />
    </b-modal>
    <!-- add dialog -->
    <b-modal id="modal-add" size="md" title="Add new option" :hide-footer="true" hide-header-close>
      <Add v-on:close-dialog="refresh" />
    </b-modal>
    <b-card v-if="options" header="Options" class="  shadow-none mt-5 mb-5">
      <table class="table table-light">
        ...
        <tr v-for="(option, index) in options" :key="index">
          ...
          <td>
            ...
            <b-button variant="light" @click="suboptions(option.id)">
...
export default {
  ...
  methods: {
    get() {
      axios
        .get("admin/options/"+this.$route.params.id)
        .then((res) => {
    ...
    suboptions(id){
      this.$router.push("/admin/service/step/option/"+id);
    },
    refresh() {
