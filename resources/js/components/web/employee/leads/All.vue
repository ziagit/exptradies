<template>
        ...
        <b-card  no-body v-for="(post, index) in posts.data" :key="index" class="overflow-hidden shadow-none my-3 " >
          ...
          <b-row no-gutters>
              <b-col md="2">
                ...
                <b-card-img v-else src="/images/placeholder.png" alt="Image" class="rounded-0 rounded-0 in-phone-d-none"></b-card-img>
              </b-col>
              <b-col md="10">
                <b-card-body :title="post.employee?post.employee.last_name:'Not found'" :sub-title="buildTitle(post)">
                  <b-card-text class="two-lines-text">
  ...
  export default {
    ...
    methods: {
      ...
      buildDate(post) {
        return "Posted on "+formatter.formatDate(post.created_at);
      },
      buildTitle(post){
